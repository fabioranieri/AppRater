...
public class AppRater {
    ...
    private static void showRateAlertDialog(final Context context, final SharedPreferences.Editor editor) {
        ...
        builder.theme(isDark ? Theme.DARK: Theme.LIGHT);
        ...
        ApplicationRatingInfo ratingInfo = ApplicationRatingInfo.createApplicationInfo(context);
        builder.title(String.format(context.getString(R.string.dialog_title), ratingInfo.getApplicationName()));

        builder.content(context.getString(R.string.rate_message));

        builder.positiveText(context.getString(R.string.rate));
        builder.neutralText(context.getString(R.string.later));
        if (!hideNoButton) {
            builder.negativeText(context.getString(R.string.no_thanks));
        }
